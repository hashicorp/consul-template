...
### Nginx
Nginx is popular open source web server, reverse proxy, and load balancer. You can read more about nginx's configuration file syntax and all the different available nginx modules in its [documentation](http://nginx.org/en/docs/), but here is an example template for rendering an nginx configuration file with Consul Template:
